...
private final BookingReferenceGenerator bookingReferenceGenerator;
    ...
    public Response findbookingBookingRefrence(String reference){
        BookingDto bookingdto = this.bookingRepository.findByBookingReference(reference)
                .map(bookingMapper::bookingToBookingDto).orElseThrow(()->new NotFoundException("booking refrence :"+reference+" not found"));

        return Response.builder()
                .status(200)
                .message("bookings with  refrence :"+ reference +", retrieved successfully")
                .booking(bookingdto)
                .build();
    ...
    public Response saveBooking(BookingDto bookingDto,Long userId,Long roomId){
      ...
      //  Payment payment=this.paymentRepository.findById(paymentId).orElseThrow(()->new NotFoundException("payment with ID "+paymentId+" not found"));
       ...
       // booking.setPayment(payment);
        ...
        // Save booking reference to audit table after successful booking creation
        bookingReferenceGenerator.saveBookingReferenceToAuditTable(bookingReference);
...
log.info("payment link is :"+paymentLink);
      NotificationDto notificationDto=NotificationDto.builder()
              ...
              .subject( room.getHotel().getHotelName() +"hotel room number :"+room.getRoomNumber()+" Booking Confirmation")
              .body("hotel booking app, Your booking has been confirmed for the periode from  "+ bookingDto.getCheckInDate()+ " to "+bookingDto.getCheckOutDate() +
...
if(days<1){
 ...
 return pricePerNight.multiply(BigDecimal.valueOf(days));
    ...
    public Response updateByBookingRefrence(String refrence,BookingUpdateDto bookingUpdateDto){
        Booking booking=bookingRepository.findByBookingReference(refrence).orElseThrow(()-> new NotFoundException("can not find booking with rference"+refrence));
        this.bookingMapper.updateBookingUpdateDtoToBooking(bookingUpdateDto,booking);
        ...
        return Response.builder()
                .status(200)
                .message("bookings with refrence "+refrence+" updated successfully")
                .booking(this.bookingMapper.bookingToBookingDto(booking))
                .build();
