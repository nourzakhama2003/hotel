...
public class UserServiceImpl implements com.nourproject.hotel.services.interfaces.UserService {
    ...
    public Response findByUserName(String userName) {
        UserDto userDto=this.userRepository.findByUserNameIgnoreCase(userName.trim()).map(userMapper::userToUserDto).orElseThrow(()->new NotFoundException("user with userName "+userName+"notFound"));
        return Response.builder()
                .status(200)
                .message("user getted Successfully")
                .user(userDto)
    ...
    public User getByUserName(String userName){
        return this.userRepository.findByUserNameIgnoreCase(userName).orElseThrow(()->new NotFoundException("user with userName admin not found"));
    }
    ...
    public Response createOrUpdateUser(UserDto userDto) {
        ...
        if (existingUser == null && userDto.getUserName() != null && !userDto.getUserName().trim().isEmpty()) {
            existingUser = this.userRepository.findByUserNameIgnoreCase(userDto.getUserName()).orElse(null);
        }
